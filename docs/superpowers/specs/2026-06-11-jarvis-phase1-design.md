...
| Phase 2 | 语音对话（浏览器收音输入 + index-tts 贾维斯音色输出） | 否，另立 spec |
...
│  workspace/ 贾维斯工作目录（AGENTS.md = 人格 + 规则 + 高危清单）           │
└────────────────────────────────────────────────────────────────────────┘
...
- 视觉：深空黑底 + 全息青蓝光效 + 扫描线/网格背景；中央反应堆呼吸圆环（贾维斯
  思考时加速旋转）；开场上线动画。
- 布局：
...
├── web/                 # 科幻控制台静态文件
├── cli/jarvis           # CLI 脚本
├── workspace/           # 贾维斯工作目录（AGENTS.md / memory.md）
├── tests/
├── .env.example
