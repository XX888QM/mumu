...
像电影一样：在房间里喊"Jarvis"，说出指令，贾维斯应声接活、出声答复；
任务办完用贾维斯音色主动播报；高危授权可以用嘴批准。全链路本地推理，零额外费用。
...
| 唤醒词 | openwakeword 0.6.0 可装，模型库含预训练 **hey_jarvis** 模型，无需注册账号 |
...
| 组件 | 位置/运行环境 | 职责 |
|------|--------------|------|
| voice/daemon.py | `.venv-voice`（新建：openwakeword+faster-whisper+silero-vad+sounddevice+httpx+websockets） | 主循环状态机：LISTEN→WAKE→RECORD→TRANSCRIBE→DISPATCH→SPEAK |
...
| .env 增量 | — | VOICE_ENABLED/VOICE_REF（默认 index-tts examples 男声，换贾维斯音色=替换 workspace/voice/jarvis_ref.wav）/ASR_MODEL(默认 large-v3-turbo)/WAKE_THRESHOLD/TTS_PORT=8778 |
...
3. openwakeword 加载 hey_jarvis 模型 → 用 TTS 合成的 "hey jarvis" 音频喂入 → 断言分数过阈值
4. 预合成缓存生成齐全
...
| 风险 | 对策 |
|------|------|
| hey_jarvis 对中式发音灵敏度 | WAKE_THRESHOLD 可调；自测用 TTS 多发音变体校准；实在不行换 openwakeword 自训词 |
...
7. 真人实测项（交付后大哥配合 1 分钟）：首跑允许麦克风 → 喊"Jarvis"下一条真指令。
