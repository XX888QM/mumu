...
SR = 16000
BLOCK = 1280  # openwakeword 推荐 80ms 帧


def test_wake_detector_real_model():
    """hey_jarvis 真模型（onnx）：喂静音帧得到 [0,1] 浮点分；reset 可用。"""
    det = WakeDetector(threshold=0.5)
    ...
    det.reset()
    # reset 后还能继续喂；float 输入也接受（内部转 int16）
